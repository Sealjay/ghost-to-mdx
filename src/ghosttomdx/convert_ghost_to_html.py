...
and exporting html and yaml files for use with a static site generator.
"""
...
def convert_ghost_export_to_html():
    ...
    convert_settings(export_folder_path, ghost_settings)
    df_tags = convert_tags(export_folder_path, ghost_tags)
    df_users = convert_users(export_folder_path, ghost_users)
...
def convert_settings(export_folder_path, ghost_settings):
    settings_we_care_about = clean_and_extract_settings(ghost_settings)
    convert_to_yaml(export_folder_path, "settings", settings_we_care_about)


def convert_tags(export_folder_path, ghost_tags):
    df_tags, dict_tags = extract_tags(ghost_tags)
    convert_to_yaml(export_folder_path, "tags", dict_tags)
    set_tag_index_to_id(df_tags)
    return df_tags


def convert_users(export_folder_path, ghost_users):
    df_users, dict_users = extract_users(ghost_users)
    convert_to_yaml(export_folder_path, "users", dict_users)
    return df_users
...
def convert_pages(export_folder_path, df_users, ghost_pages):
    ...
    dict_pages = convert_to_html(export_folder_path, dict_pages, "pages")
    # TODO: Replace __GHOST_URL__ with a base site URL
    convert_to_yaml(export_folder_path, "pages", dict_pages)
...
def convert_posts(export_folder_path, ghost_posts_tags, df_tags, df_users, ghost_posts):
    ...
    dict_posts = convert_to_html(export_folder_path, dict_posts, "posts")
    convert_to_yaml(export_folder_path, "posts", dict_posts)
...
def convert_to_yaml(export_name, filename, dict_data):
    with open(f"{export_name}{filename}.yaml", "w") as f:
